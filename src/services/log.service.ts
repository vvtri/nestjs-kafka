...
export class LogService {
  ...
  errorParseBySchemaButSchemaRegistryNotfound(topic: string) {
    this.logger.error(
      `Subscribe to ${topic} and parse by schema but schema registry not found`,
    );
  }

  errorSendMessageWithSchemaIdButSchemaRegistryNotFound(
    topic: string,
    schemaId: number,
  ) {
    this.logger.error(
      `Send message to ${topic} with schemaID: ${schemaId} but schema registry not found`,
    );
  }
  ...
  errorWhenSubscribing(error: any) {
    this.logger.error(`Subscribe failed, error`, error);
  }
